...
// The zSLC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zSLC is a hash of the serial
...
{
    //Create an output returning the zSLC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZSLCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zSLC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zSLC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZSLCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zSLC output", __func__);
...
{
    CzSLCTracker* zslcTracker = pwallet->zslcTracker.get();
    CMintMeta meta;
    if (!zslcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zslcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a SLC stake is the outpoint
