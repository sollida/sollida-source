...
#ifndef sollida_ZSLCWALLET_H
#define sollida_ZSLCWALLET_H
...
public:
    CzSLCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSLC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSLC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //sollida_ZSLCWALLET_H
