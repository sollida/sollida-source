...
public:
    ...
    bool WriteZSLCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSLCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSLCSeed_deprecated(uint256& seed);
    bool EraseZSLCSeed();
    bool EraseZSLCSeed_deprecated();

    bool WriteZSLCCount(const uint32_t& nCount);
    bool ReadZSLCCount(uint32_t& nCount);
