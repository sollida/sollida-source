...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a sollida address (e.g. %1)").arg("S7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no sollida: URI
...
{
    // Convert sollida:// to sollida:
    //
    //    Cannot handle this later, because sollida:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open sollida.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "sollida.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for sollida.lnk
...
{
    return GetAutostartDir() / "sollida.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a sollida.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=sollida\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the sollida app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add sollida app to startup item list
