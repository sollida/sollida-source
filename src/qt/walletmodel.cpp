...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered sollida address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SLC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal sollida:URI (sollida:XyZ...?message=example)
            {
...
static void NotifyzSLCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSLCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSLCReset.connect(boost::bind(NotifyzSLCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSLCReset.disconnect(boost::bind(NotifyzSLCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zslcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
