...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizesollidaAmount"))
        settings.setValue("nAnonymizesollidaAmount", 1000);

    nAnonymizesollidaAmount = settings.value("nAnonymizesollidaAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizesollidaAmount"))
        SoftSetArg("-anonymizesollidaamount", settings.value("nAnonymizesollidaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in sollida.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizesollidaAmount:
            return QVariant(nAnonymizesollidaAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizesollidaAmount:
            nAnonymizesollidaAmount = value.toInt();
            settings.setValue("nAnonymizesollidaAmount", nAnonymizesollidaAmount);
            emit anonymizesollidaAmountChanged(nAnonymizesollidaAmount);
            break;
        case CoinControlFeatures:
