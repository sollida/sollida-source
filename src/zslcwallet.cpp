...
CzSLCWallet::CzSLCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zslc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSLCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSLCSeed_deprecated()) {
                    LogPrintf("%s: Updated zSLC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zslc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zslc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSLCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSLCCount(nCountLastUsed);
    else if (!walletdb.ReadZSLCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSLCWallet::Lock()
{
    seedMaster = 0;
}

void CzSLCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSLCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSLC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSLCWallet::LoadMintPoolFromDB()
{
...
void CzSLCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSLCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSLCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zslcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSLCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSLC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zslcTracker which also adds to database
    pwalletMain->zslcTracker->Add(dMint, true);
...
void CzSLCWallet::SeedToZSLC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSLCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSLCWallet::GenerateDeterministicZSLC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSLCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSLCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
